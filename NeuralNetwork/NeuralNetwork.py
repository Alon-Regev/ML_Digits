...
class NeuralNetwork():
    def __init__(self, layers_sizes: list, learning_rate: float = LEARNING_RATE):
        ...
        for i in range(self.layer_count):
            self.weights.append(np.random.rand(layers_sizes[i], layers_sizes[i + 1]))
            self.biases.append(np.random.rand(layers_sizes[i + 1]))
    ...
    def fit(self, training_data: list, iterations: int = 1000) -> None:
        ...
        for _ in range(iterations):
            ...
            for i in range(self.layer_count)[::-1]:
                ...
                dEdf = dEdo * sigmoid_derivative(f_l[i])
                dEdb = dEdf
    ...
    def test(self, test_data: dict) -> float:
        """
